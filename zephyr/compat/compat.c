...
#define STUB()                                                         \
	LOG_ERR( "lua/zephyrcompat/compat.c: %s invoked!", __func__ ); \
	while ( 1 ) abort();
...
{
	// TODO: This is SO NOT a standards-conformant (or even good)
	// implementation
	//  (no infs, no nan, no e+, etc.) but good enough for 1, 1.2, and .2
	//  The newlib and others impl involve dynamic allocation (!?!?) and it
	//  was expedient to just write one
...
{
	// TODO: Need to reimplement these so that they don't cause MPU fault in
	// userland
...
__weak void lua_compat_writestring( void* ud, char const* s, const int l )
{
	LOG_INF( "lua: %.*s", l, s );
}

__weak void lua_compat_writestringerror( void* ud, char const* f, char const* s )
{
